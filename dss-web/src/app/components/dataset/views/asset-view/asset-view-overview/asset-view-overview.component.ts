...
import {AssetDetails, AssetEntityClassification, ReplicationProperty} from '../../../../../models/asset-property';
import {
...
export class AssetViewOverviewComponent implements OnChanges, OnDestroy {
  ...
  @Input() isSensitivityProfilerRan = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes && changes['assetDetails'] && changes['assetDetails'].currentValue) {
      ...
      if(this.assetDetails.entity.attributes.replicatedTo || this.assetDetails.entity.attributes.replicatedFrom) {
        this.toClusterIds = [];
        this.fromClusterId = null;
        let repProps:ReplicationProperty[] = [];
        if(this.assetDetails.entity.attributes.replicatedFrom)
          repProps.push.apply(repProps, this.assetDetails.entity.attributes.replicatedFrom)
        if(this.assetDetails.entity.attributes.replicatedTo)
          repProps.push.apply(repProps, this.assetDetails.entity.attributes.replicatedTo)
        this.findReplicationClusterDetails(repProps)
      }
    }
  }

  findReplicationClusterDetails(repProps:ReplicationProperty[]) {
    this.showLineage=false;
    let requestObservables:Observable<any>[] = [];
    if(!this.lakes.length)
      requestObservables.push(this.lakeService.listWithClusters('lake'))
    repProps.forEach(repProp => requestObservables.push(this.assetService.getDetails(this.clusterId,repProp.guid)));
    Observable.forkJoin(requestObservables).subscribe(resp => {
      const extractLakeInfo = objs => {
        objs.forEach(obj => {
      ...
      resp.forEach((res,i) => {
        if(!this.lakes.length && i === 0) return extractLakeInfo(res);
        const names = res.entity.attributes.fullName.split("$");
        const clusterId = this.lakes.find(l=>(l.dcName == names[0] && l.name == names[1])).clusterId
        if(!this.fromClusterId && this.assetDetails.entity.attributes.replicatedFrom) // only single replicated from node possible
          this.fromClusterId = clusterId;
        else this.toClusterIds.push(clusterId)
      })
  ...
  private createTopAssetsChart(profilerMetricResponse: ProfilerMetricResponse) {
    ...
    if (profilerMetricResponse.status) {
      ...
      data = Object.keys(secureAssetAccessUsers.accessCounts).map(k => {
        ...
        minValue = Math.min(minValue, secureAssetAccessUsers.accessCounts[k]);
  ...
  private getAuthorisedAndUnAuthorisedData(startDate: string, endDate: string) {
    ...
    if (assetName && dbName && startDate && endDate) {
      ...
      .subscribe((resp: any) => {
        this.createAuthorisedAndUnAuthorisedChart(resp.body.data);
      });
  ...
  private getAuditAccessData(startDate: string, endDate: string) {
    ...
    if (assetName && dbName && startDate && endDate) {
      ...
      .subscribe((resp: any) => {
        this.createAuditAccessChart(resp.body.data);
      });
