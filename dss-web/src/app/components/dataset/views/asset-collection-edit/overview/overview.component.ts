...
import {RichDatasetService} from 'app/components/dataset/services/RichDatasetService';
...
export class OverviewComponent implements OnChanges, OnDestroy {
  ...
  private createDistributionByTagChart() {
    let distributionByTagData = [], maxValue = 0;
  ...
  private createSensitiveNonSensitiveChart() {
    ...
    if (metrics.status && this.noOfTablesProfiledBySensitivityProfiler > 0) {
      ...
      data = [
        {key: 'Sensitive', y: sensitiveDataValue},
        {key: 'Non Sensitive', y:  nonSensitiveDataValue}
      ];
    }
    nv.addGraph(() => {
  ...
  private getQuiresRunningSensitiveDataChart(startDate: string, endDate: string) {
    ...
    DomUtils.removeAllChildNodes(this.quiresRunningSensitiveData.nativeElement);
    this.quiresRunningSensitiveData.nativeElement.classList.add('loader');

    const subscription = this.profileService.assetCollectionStats(metricsRequests)
      .finally(() => this.quiresRunningSensitiveData.nativeElement.classList.remove('loader'))
      .subscribe(assetCollectionDashboard => {
