...
import models.SocialEntities.Comment
...
class Comments @Inject()(commentsRepo:CommentsRepo,
    ...
    request.body
      ...
      .map { comment =>
        ...
        if(!objectTypes.contains(comment.objectType)) {
          ...
          commentsRepo
            .add(comment,  new DssUser(request.user))
            .map { comment =>
...
//  }
  ...
  def getByObjectRef(objectId: String, objectType: String) = Action.async { req =>
    ...
    if(!objectTypes.contains(objectType)){
      ...
      commentsRepo.findByObjectRef(objectId.toLong, objectType, req.getQueryString("offset").getOrElse("0").toLong, req.getQueryString("size").getOrElse("20").toLong)
  ...
  def getCommentsCount(objectId: Long, objectType: String) = Action.async { req =>
    ...
    if(!objectTypes.contains(objectType)){
      ...
      commentsRepo.getCommentsCount(objectId, objectType)
        .map { commentsCount =>
          Ok(Json.obj("totalComments"->commentsCount))
        }
  ...
  def getByParentId(parentId: String) = Action.async { req =>
    ...
    commentsRepo.findByParentId(parentId.toLong)
      .map(seq => Ok(Json.toJson(seq)))
  ...
  def deleteCommentById(commentId: String) = AuthenticatedAction.async { req =>
    ...
    commentsRepo.deleteCommentById(commentId.toLong, new DssUser(req.user))
      .map{ num =>Ok(s"Success: ${num} row/rows deleted")}
