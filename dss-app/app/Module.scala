...
class Module extends AbstractModule {
  ...
  def configure() = {
    ...
    bind(classOf[CredentialInterface]).to(classOf[CredentialInterfaceImpl])
  ...
  @Singleton
  @Named("atlasService1")
  def provideAtlasService1Service(implicit ws: WSClient, configuration: Configuration
                                  , urlService: UrlService, credentialInterface: CredentialInterface): AtlasService = {
    implicit val kProxyWsClient = KnoxProxyWsClient(ws, configuration.underlying)
    new AtlasService(configuration.underlying, urlService, credentialInterface)
  }


  @Provides
  @Singleton
  def provideKnoxProxyWsClient(implicit ws: WSClient, configuration: Configuration): KnoxProxyWsClient = {
    KnoxProxyWsClient(ws, configuration.underlying)
  }
